	...
	while (true)
	{
		...
		if (overlapped->ID == READ)
		{
			...
			while (true)
			{
				...
				if (wHeaderLength != ptr->RecvBuffer->Peek(newMessage.GetBufferPtr(), wHeaderLength))
					break;
		...
		else if (overlapped->ID == WRITE)
		{
			...
			InterlockedExchange(&ptr->bIsSend, FALSE);
...
bool joshua::NetworkLibrary::PostSend(st_SESSION* session)
{
	...
	{
		if (session->SendBuffer->GetUseSize() == 0 || session->SessionID == -1)
		{
		...
		//if ((session->SendBuffer->GetUseSize() / 8) <= MAX_PACKET_COUNT)
		{
			...
			{
				if (session->SendBuffer->GetUseSize() < 8)
					break;
				CMessage* packet;
		...
		if (retval == SOCKET_ERROR)
		{
			if (WSAGetLastError() != ERROR_IO_PENDING)
			{
				InterlockedExchange(&(session->bIsSend), FALSE);
				//SYSLOGCLASS* _pLog = SYSLOGCLASS::GetInstance();
				//_pLog->LOG(L"SERVER", LOG_LEVEL::e_ERROR, L"WSASend() error. SessionID : %d, ErrorCode : %d\n", session->SessionID, WSAGetLastError());
				DisconnectSession(session->SessionID);
				InterlockedDecrement(&session->dwIOCount);
